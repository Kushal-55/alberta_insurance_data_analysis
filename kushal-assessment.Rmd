---
title: 'Data Scientist (ID: 75523) Assessment'
...
knit: (function(inputFile, encoding) {
    ...
    rmarkdown::render(inputFile,
      encoding=encoding,
      output_file=file.path(dirname(inputFile), out_dir, 'Kushal Adhvaryu - DS Assessment.pdf'))})
---
...
1. Load any packages you require to complete this assessment. 
...
For this section, please try to use the syntax/commands used in Microsoft SQL Server. If you do not, please specify what SQL language you're using. Don't worry about small syntax changes or errors, we're interested in your overall approach. 

1. a) You have a fact table named `FactClaims`, with the following columns: `claim_id`, `customer_id`, `claim_date`, `claim_amount`, `vehicle_id`, and `claim_status`. Write a query to find the top 10 customers by total `claim_amount`, where the status is "Paid", and the claim date is in the past year. 
...
b) Alter this query to find the top 10 customers with the highest **average claim severity**. Claim severity is defined as total `claim_amount` divided by total claims. 
...
ROUND(SUM(claim_amount) / CAST(COUNT(*) AS FLOAT), 2) AS claim_severity
FROM FactClaims
WHERE claim_status = 'Paid'
AND claim_date >= DATEADD(YEAR, -1, GETDATE())
GROUP BY customer_id
...
2. You have a dimension table called `DimVehicleClass` that has columns `vehicle_id` and `vehicle_type`. You have the fact table called `FactClaims` from question 1. Write two queries to find the `vehicle_type` that has the most claims. The only output of the query that matters is the vehicle type. 

a) For this version of the query, use a **subquery**.

```sql
SELECT vehicle_type
FROM DimVehicleClass
WHERE vehicle_id = (
SELECT TOP 1 vehicle_id
FROM (
SELECT vehicle_id, COUNT(*) as claim_count
FROM FactClaims
GROUP BY vehicle_id
) AS vehicle_claims
...
b) For this version of the query, use a **join**.

```sql
SELECT TOP 1 dvc.vehicle_type
FROM FactClaims fc
INNER JOIN DimVehicleClass dvc ON fc.vehicle_id = dvc.vehicle_id
GROUP BY dvc.vehicle_type, dvc.vehicle_id
ORDER BY COUNT(*) DESC;
```

c) Which type of query is more efficient? What other factors should be considered when writing a query?

```
The JOIN query is generally more efficient than the subquery 
because:
* Joins have single pass processing and process the data in one operation, 
subqueries may require multiple passes
* Joins can better utilize indexes on both tables 
* Joins have more optimization techniques (merge joins, 
hash joins etc) whereas subqueries limit optimization options

The factors to be considered while writing queries:
* Joins should be preferred for large datasets
* Proper indexing should be applied on join columns
* Subqueries can be used to apply complex logic
* CTEs should be used where necessary
* Only query fields that are needed
```

3. a) Using the information from question 1, write a query to calculate a 30-day moving average of daily claim amounts. Store the result into a temporary table.
...
claim_day,
daily_claim_amount,
AVG(CAST(daily_claim_amount AS FLOAT)) OVER (
ORDER BY claim_day 
...
days_in_window
INTO #TempMovingAverage
FROM MovingAverages
ORDER BY claim_day;
```

b) Using the results of your temporary table from part a, create a ranking in descending order for the 30 day moving averages. 

```sql
WITH RankedMovingAverages AS (
    SELECT 
        claim_day,
        daily_claim_amount,
        moving_avg_30day,
        days_in_window,
        RANK() OVER (ORDER BY moving_avg_30day DESC) as standard_rank,
        PERCENT_RANK() OVER (ORDER BY moving_avg_30day DESC) as percentile_rank
    FROM #TempMovingAverage
    WHERE days_in_window = 30  -- Only include periods with full 30-day window
)
SELECT 
    standard_rank,
    claim_day,
